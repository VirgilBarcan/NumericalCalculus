...
public:
	...
	@param x - the vector for which we want to calculate the norm
	...
	*/
	static double EuclideanNorm(Matrix *x) {
		...
		if (x != nullptr && x->getNoOfLines() != 1) {
			norm = -1;
		}
		else {
			//calculate the norm
			for (int i = 0; i < x->getNoOfColumns(); ++i) {
				norm += pow(x->getElementAt(0, i), 2);
			}
	...
	*/
	static double pNorm(Matrix *x, double p) {
		...
		if (x != nullptr && x->getNoOfLines() != 1) {
			...
			//calculate the norm
			for (int i = 0; i < x->getNoOfColumns(); ++i) {
				norm += pow(fabs(x->getElementAt(0, i)), p);
			}
